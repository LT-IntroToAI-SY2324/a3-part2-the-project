...
from album import album_db
from match import match
from typing import List, Tuple, Callable, Any

# The projection functions, that give us access to certain parts of a "movie" (a tuple)
def get_album(album: Tuple[str, str, int, List[str]]) -> str:
    return album[0]


def get_artist(album: Tuple[str, str, int, List[str]]) -> str:
    return album[1]


def get_year(album: Tuple[str, str, int, List[str]]) -> int:
    return album[2]


def get_song(album: Tuple[str, str, int, List[str]]) -> List[str]:
    return album[3]
...
def album_by_year_range(matches: List[str]) -> List[str]:
    ...
    for album in album_db:
        if int(matches[0]) <= get_year(album) <= int(matches[1]):
            # print(get_title(album))
            results.append(get_title(album))
    # print(results)
    return results


def album_before_year(matches: List[str]) -> List[str]:
    """Finds all album made before the passed in year
    """
    results = []
    # print(matches)
    for album in album_db:
        if get_year(album) < int(matches[0]):
            # print(get_title(album))
            results.append(get_artist(album))
    # print(results)
    return results
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("what is a song made by %"), title_by_artist),
    (str.split("what songs were made by %"), title_by_artist),
    (str.split("when was % made"), year_by_title),
    (str.split("in what album did % appear in"), title_by_artist),
    (["bye"], bye_action),
]
...
if __name__ == "__main__":
    assert isinstance(album_by_year(["2010"]), list), "title_by_year not returning a list"
    assert isinstance(album_by_year_range(["2000", "2010"]), list), "title_by_year_range not returning a list"
    assert isinstance(album_before_year(["2020"]), list), "title_before_year not returning a list"
    assert isinstance(album_after_year(["1900"]), list), "title_after_year not returning a list"
    assert isinstance(title_by_album(["reputation"]), list), "title_by_album not returning a list"
    assert isinstance(title_by_artist(["Amy Whinehouse"]), list), "title_by_artist not returning a list"
    assert isinstance(songs_by_artist(["goodbye yellow brick road"]), list), "songs_by_artist not returning a list"
    assert isinstance(year_by_title([""]), list), "year_by_title not returning a list"
    assert isinstance(title_by_artist(["brunomars"]), list), "title_by_artist not returning a list"
    
    assert sorted(album_by_year(["2017"])) == sorted(
        ["reputation"]
    ), "failed title_by_year test"
    assert sorted(album_by_year_range(["1990", "2000"])) == sorted(
        ["jagged little pill"]
